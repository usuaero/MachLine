...
module panel_mod
    ...
    type panel
        ...
        contains
            ...
            procedure :: calc_hH113_supersonic_subinc_adjoint => panel_calc_hH113_supersonic_subinc_adjoint
...
contains
    ...
    subroutine panel_calc_remaining_integrals(this, geom, influence_type, freestream, mirror_panel, int, dod_info)
        ...
        !!! Nate turned calc recursions (below this comment) off because the resulting terms aren't used 
        !!!!in any of the current formulations
    ...
    subroutine panel_calc_d_n_hat_g(this)
        ...
        type(sparse_matrix) :: t_cross_d_n, d_d_g, d_norm_d_g_times_d_g, d_t_hat_g
        ...
        do i=1,this%N
            ...
            ! broadcast t_hat_g crossed with d_n_g:  t_cross_d_n
            t_cross_d_n = this%d_n_g%broadcast_vector_cross_element(t_hat_g)
            ...
            call this%d_n_hat_g(i)%sparse_add(t_cross_d_n) 

            ! deallocate for the next iter
            deallocate(t_cross_d_n%columns)
    ...
    subroutine  panel_calc_velocity_influences_adjoint(this, P, d_P, freestream, d_v_d_M_space)
        ...
        if (dod_info%in_dod .and. this%A > 0.) then
            ...
            call this%calc_integrals_adjoint(geom, 'velocity', int, freestream, mirror_panel, dod_info)
    ...
    subroutine panel_calc_integrals_adjoint(this,geom,influence_type,int,freestream, mirror_panel, dod_info) 
        ...
        if (freestream%supersonic) then
            if (int%r < 0) then
                ...
                call this%calc_basic_F_integrals_supersonic_subinc_adjoint(geom, dod_info, freestream, mirror_panel, int)
                call this%calc_hH113_supersonic_subinc_adjoint(geom, dod_info, freestream, mirror_panel, int)
            end if
        else
    ...
    subroutine panel_calc_basic_F_integrals_subsonic_adjoint(this, geom, freestream, mirror_panel, int)
        ...
        do i=1,this%N
            ...
            if (sign(1., geom%l1(i)) /= sign(1., geom%l2(i))) then
                ...
                ! calc d_(L1*L2)  (more efficient version)
    ...
    subroutine panel_calc_basic_F_integrals_supersonic_subinc_adjoint(this, geom, dod_info, freestream, mirror_panel, int)
        ...
        hi_d_low, hi_d_low2, hi_d_low3, hi_d_low4, hi_d_low5, &
        d_eps, d_eps_term2, &
        d_F111_term1, d_F111_term2, d_F111_term3, d_F111_term4, d_F111_term5, d_F111_term6,&
        d_atan_1, d_atan_2, d_atan_3
        ...
        do i=1,this%N
            ...
            if (dod_info%edges_in_dod(i)) then
                ...
                ! Mach wedge
                if (geom%R1(i) == 0. .and. geom%R2(i) == 0) then
                    ! write(*,*) "mach wedge case F integral"
                    ! F(1,1,1)
                    int%F111(i) = pi/s_b
                    ...
                    ! Calculate F factors for supersonic or nearly sonic
                    if (b > 0.) then
                        ...
                        call low_d_hi%sparse_subtract(low_d_hi4)
                        ...
                        call low_d_hi%sparse_add(low_d_hi4)
                        call low_d_hi%sparse_add(low_d_hi5)
                        
                        call low_d_hi%broadcast_element_times_scalar(geom%g2(i))
                        
                        ! calculate high d low F2
                        call hi_d_low%init_from_sparse_vector(geom%d_g2(i))
                        call hi_d_low%broadcast_element_times_scalar(b*geom%R1(i)*geom%R2(i) + geom%l1(i)*geom%l2(i))
                        ...
                        if (abs(F2) > 125.0*abs(s_b*F1)) then
                            ...
                            ! calc d_eps
                            call d_eps%init_from_sparse_vector(d_F1)
                            call d_eps%broadcast_element_times_scalar(F2)
                            
                            call d_eps_term2%init_from_sparse_vector(d_F2)
                            call d_eps_term2%broadcast_element_times_scalar(F1)
                            ...
                            call d_F111_term3%init_from_sparse_vector(this%d_b(i))
                            call d_F111_term3%broadcast_element_times_scalar(b*eps*eps2*(-1.)*eps2/5.)
                            
                            call d_F111_term4%init_from_sparse_vector(d_eps)
                            call d_F111_term4%broadcast_element_times_scalar(b*eps*eps2*(-2.)*b*eps/5.)

                            call d_F111_term5%init_from_sparse_vector(this%d_b(i))
                            call d_F111_term5%broadcast_element_times_scalar(b*eps*eps2*2.*b*eps2*eps2/7.)

                            call d_F111_term6%init_from_sparse_vector(d_eps)
                            call d_F111_term6%broadcast_element_times_scalar(b*eps*eps2*4.*b*b*eps2*eps/7.)
                            
                            ! calc d_F111(i)
                            call int%d_F111(i)%init_from_sparse_vector(d_F111_term1)
                            call int%d_F111(i)%sparse_add(d_F111_term2)
                            call int%d_F111(i)%sparse_add(d_F111_term3)
                            call int%d_F111(i)%sparse_add(d_F111_term4)
                            call int%d_F111(i)%sparse_add(d_F111_term5)
                            call int%d_F111(i)%sparse_add(d_F111_term6)
                            call int%d_F111(i)%sparse_subtract(d_eps)
                            
                            deallocate(d_eps%elements, d_eps_term2%elements, d_F111_term1%elements, d_F111_term2%elements, &
                            d_F111_term3%elements, d_F111_term4%elements, d_F111_term5%elements, d_F111_term6%elements)
                            ...
                            deallocate(d_F1%elements, d_F2%elements)
                            deallocate(d_atan_1%elements, d_atan_2%elements, d_atan_3%elements)
                        ...
                        ! calculate F1 and F2 for (b > 0.) = false case
                        F1 = (geom%R2(i) - geom%R1(i))*(geom%R2(i) + geom%R1(i)) / (geom%l1(i)*geom%R2(i) + geom%l2(i)*geom%R1(i))
                        F2 = (geom%g2(i) - geom%l1(i)**2 - geom%l2(i)**2) / (b*geom%R1(i)*geom%R2(i) - geom%l1(i)*geom%l2(i))
                        
                        
                        ! before calculating d_F1 and d_F2, check to see if the edge is nearly sonic
                        ! If Nearly-sonic edge
                        if (abs(F2) > 125.0*abs(s_b*F1)) then
                            ! write(*,*) "nearly sonic? how can it be nearly sonic?"
                            
                            !!!!!!!!!! calculate d_F1 and d_F2 for the nearly sonic, (b > 0) = false case!!!!!!!!!
                            ! quotient rule low_d_hi d_F1
                            call low_d_hi%init_from_sparse_vector(geom%d_R2(i))
                            call low_d_hi%sparse_subtract(geom%d_R1(i))
                            call low_d_hi%broadcast_element_times_scalar(geom%R2(i) + geom%R1(i))
                            ...
                            !!!!!!!!!! end d_F1 if b > 0 is false, nearly sonic !!!!!!!!!!
                            
                            
                            !!!!!!!! d_F2 if b > 0. is false, nearly sonic !!!!!!!!!!
                            ...
                            call hi_d_low%broadcast_element_times_scalar(geom%g2(i) - geom%l1(i)**2 - geom%l2(i)**2)
                            
                            ! calc d_F2
                            call d_F2%init_from_sparse_vector(low_d_hi)
                            call d_F2%sparse_subtract(hi_d_low)
                            call d_F2%broadcast_element_times_scalar(1./(this%b(i)*geom%R1(i)*geom%R2(i) &
                                                                                    - geom%l1(i)*geom%l2(i))**2)
                            ...
                            !!!!!!!! end d_F2 if b > 0. is false, nearly sonic !!!!!!!!!!

                            !!!!!!!!!! calculate d_F111, for (b > 0) = false, nearly sonic
                            ...
                            if (F1 /= 0. .and. F2 /= 0.) then
                                ...
                                call int%d_F111(i)%init_from_sparse_vector(low_d_hi)
                                call int%d_F111(i)%sparse_subtract(hi_d_low)
                                call int%d_F111(i)%broadcast_element_times_scalar(-sign(1., geom%v_eta(i))/(s_b*s_b))
                           
                                deallocate(low_d_hi%elements, hi_d_low%elements)
                            else
                                if (verbose) write(*,*) "!!! Detected evaluation point on perimeter of panel. &
                                                                                            Solution may be affected."
                            end if
                        
                            ! deallocate stuff
                            deallocate(d_F1_term1%elements, d_F1_term2%elements, d_F2_term1%elements, d_F2_term2%elements,&
                                        d_eps%elements, d_eps_term2%elements)
                            deallocate(d_F1%elements, d_F2%elements)
                        ...
                        end if ! end of nearly sonic, (b > 0) = false   if statement

                    end if ! end of (b > 0.) if statementend of 

                end if  ! end of (geom%R1(i) == 0. .and. geom%R2(i) == 0) if statement
            
            else ! edge not in dod
                
                ! F111(i) is zero, therefore sensitivity is zero
                call int%d_F111(i)%init(this%adjoint_size) 
            
            end if ! end of edge in dod if statement


            
            !!!!!!!!!! some unused  higher order stuff !!!!!!!!!!!!!!
            ! ! Check
            ! if (this%order == 2) then
            !     if (abs(geom%v_xi(i)*int%F211(i) + geom%v_eta(i)*int%F121(i) - geom%a(i)*int%F111(i)) > 1.e-12) then
            !         write(*,*) "!!! Calculation of F(2,1,1) and F(1,2,1) failed. Please submit a bug report on GitHub."
    ...
    subroutine panel_calc_hH113_supersonic_subinc_adjoint(this, geom, dod_info, freestream, mirror_panel, int)
        ! Calculates hH(1,1,3) for a subinclined panel in supersonic flow.
        ! Taken from Ehlers et al. (1979) Appendix E.
        ...
        type(sparse_vector) ::  low_d_hi, low_d_hi2, low_d_hi3, low_d_hi4, low_d_hi5, &
        hi_d_low, hi_d_low2, hi_d_low3, hi_d_low4, hi_d_low5, &
        d_y2, d_y3, &
        d_z, d_z2, d_z3, d_z4,&
        d_F1, d_F2,&
        ...
        ! Calculate hH(1,1,3) (Ehlers Eq. (E18))
        int%hH113 = 0.
        call int%d_hH113%init(this%adjoint_size)
            ...
            if (dod_info%edges_in_dod(i)) then
                ...
                if (abs(geom%h) > 1.e-12) then
                    ...
                    if (geom%R1(i) == 0. .and. geom%R2(i) == 0.) then
                        int%hH113 = int%hH113 + pi*sign(1., geom%h*geom%v_xi(i))
                        ! write(*,*) "mach wedge"
                        !!!NOTE: if R1 and R2 == 0, d_hH113 contribution for this edge is zero !!!
                    else
                        ...
                        if (b > 0.) then
                            ...
                            !!!!!!!!!!!!!!!!!! The following is duplicate work from calculating supersonic F111 integrals !!!!!!!!!
                            !!!!!!!!!!!!!!!!!! repeated work here to keep F and hH113 subroutines separate !!!!!!!!!!!!!!!!!!!!
                            !!!!!!!!!!!!!!!!!! consider an elegant way to combine adjoint d_F111 and d_hH113 
                            !!!!!!!!!!!!!!!!!! so these d_F1 and d_F2 calcs are only done once
                        ...
                        ! Calculate F factors for subsonic edge
                            ...
                            ! quotient rule low_d_hi d_F1
                            call low_d_hi%init_from_sparse_vector(geom%d_dR(i))
                            call low_d_hi%broadcast_element_times_scalar(geom%R2(i) + geom%R1(i))
                            
                            call low_d_hi2%init_from_sparse_vector(geom%d_R2(i))
                            call low_d_hi2%sparse_add(geom%d_R1(i))
                            call low_d_hi2%broadcast_element_times_scalar(geom%dR(i))
                            ...
                            call hi_d_low%broadcast_element_times_scalar(geom%dR(i)*(geom%R2(i) + geom%R1(i)))
                            ...
                            !!!!!!!! d_F2 if b > 0. is false, nearly sonic !!!!!!!!!!
                            ! (geom%g2(i) - geom%l1(i)**2 - geom%l2(i)**2) &
                            !      / (b*geom%R1(i)*geom%R2(i) - geom%l1(i)*geom%l2(i))
                            ...
                            call low_d_hi%broadcast_element_times_scalar(b*geom%R1(i)*geom%R2(i) - geom%l1(i)*geom%l2(i))
                            ...
                            call hi_d_low2%init_from_sparse_vector(geom%d_R1(i))
                            call hi_d_low2%broadcast_element_times_scalar(b*geom%R2(i))
                            
                            call hi_d_low3%init_from_sparse_vector(geom%d_R2(i))
                            call hi_d_low3%broadcast_element_times_scalar(b*geom%R1(i))
                            ...
                            call d_F2%init_from_sparse_vector(low_d_hi)
                            call d_F2%sparse_subtract(hi_d_low)
                            call d_F2%broadcast_element_times_scalar(1./(b*geom%R1(i)*geom%R2(i) - geom%l1(i)*geom%l2(i))**2)
                        ...
                        call d_hH113_i%init_from_sparse_vector(geom%d_h)
                        call d_hH113_i%broadcast_element_times_scalar(geom%a(i)*F1)
                        
                        call d_y2%init_from_sparse_vector(geom%d_a(i))
                        call d_y2%broadcast_element_times_scalar(geom%h*F1)

                        call d_y3%init_from_sparse_vector(d_F1)
                        call d_y3%broadcast_element_times_scalar(geom%h*geom%a(i))

                        call d_hH113_i%sparse_add(d_y2)
                        call d_hH113_i%sparse_add(d_y3)

                        call d_hH113_i%broadcast_element_times_scalar((geom%R1(i)*geom%R2(i) + geom%h2*F2)& 
                        / ((geom%h*geom%a(i)*F1)**2 + (geom%R1(i)*geom%R2(i) + geom%h2*F2)**2 ))

                        call d_z%init_from_sparse_vector(geom%d_R1(i))
                        call d_z%broadcast_element_times_scalar(geom%R2(i))

                        call d_z2%init_from_sparse_vector(geom%d_R2(i))
                        call d_z2%broadcast_element_times_scalar(geom%R1(i))

                        call d_z3%init_from_sparse_vector(geom%d_h2)
                        call d_z3%broadcast_element_times_scalar(F2)

                        call d_z4%init_from_sparse_vector(d_F2)
                        call d_z4%broadcast_element_times_scalar(geom%h2)

                        call d_z%sparse_add(d_z2)
                        call d_z%sparse_add(d_z3)
                        call d_z%sparse_add(d_z4)

                        call d_z%broadcast_element_times_scalar((-geom%h*geom%a(i)*F1)&
                        /((geom%h*geom%a(i)*F1)**2 + (geom%R1(i)*geom%R2(i) + geom%h2*F2)**2 ))

                        call d_hH113_i%sparse_add(d_z)

                        call int%d_hH113%sparse_add(d_hH113_i)

                        ! deallocate stuff
                        deallocate(d_y2%elements, d_y3%elements, &
                                d_z%elements, d_z2%elements, d_z3%elements, d_z4%elements)
                        deallocate(d_F1%elements, d_F2%elements,&
                                    d_hH113_i%elements) 
                    end if
                else
    ...
    end subroutine panel_calc_hH113_supersonic_subinc_adjoint
    ...
    function panel_assemble_v_d_M_space_adjoint(this, int, geom, freestream, mirror_panel) result(d_v_d_M)
    ...
    function panel_get_d_doublet_parameters_wrt_vars(this, mu, mirror, N_body_verts, asym_flow) result(d_mu_params)
        ...
        if (mirror) then
            ! mu_params = matmul(this%T_mu_mir, mu_verts)
            write(*,*) "!!! Cannot calculate adjoint doublet parameters for mirrored mesh yet. Quitting..."
            stop
        else
            ! mu_params = matmul(this%T_mu, mu_verts)
            call d_T_mu_3D%init_from_sparse_matrices(this%d_T_mu_rows)
    ...
    function panel_calc_adjoint_potential_influences(this, cp, freestream, mirror_panel) result(inf_adjoint)
        ...
        if (dod_info%in_dod .and. this%A > 0.) then
            ...
            if (this%has_sources) then
                !phi_s_S_space = this%assemble_phi_s_S_space(int, geom, freestream, mirror_panel)
                write(*,*) "adjoint formulations shouldn't include sources, quitting..."
                stop
            else
                allocate(phi_s_S_space(this%S_dim), source=0.)
            end if
            ...
            ! Add quadratic terms here
                ! psych! adjoint not supported for higher order formulations

            ! Convert to strength influences (Davis Eq. (4.41))
            if (mirror_panel) then
                !phi_d_M_space(1:this%M_dim) = int%s*freestream%K_inv*matmul(phi_d_mu_space, this%T_mu_mir)
                write(*,*) "mirroring not yet supported for calc_adjoint_potential_influences, quitting..."
                stop
            else
                phi_d_M_space(1:this%M_dim) = int%s*freestream%K_inv*matmul(phi_d_mu_space, this%T_mu)
            end if
            ...
            ! assemble adjoint d_phi_d_mu space

            ! create sparse vectors 
            call d_phi_d_mu_space(1)%init_from_sparse_vector(int%d_hH113)
            
            ! calc d_phi_d_mu_space(2)
            call d_phi_d_mu_space(2)%init_from_sparse_vector(int%d_hH113)
            call d_phi_d_mu_space(2)%broadcast_element_times_scalar(geom%P_ls(1))

            call term1%init_from_sparse_vector(geom%d_P_ls(1))
            call term1%broadcast_element_times_scalar(int%hH113)

            call term2%init_from_sparse_vector(geom%d_h)
            call term2%broadcast_element_times_scalar(int%H213)

            call term3%init_from_sparse_vector(int%d_H213)
            call term3%broadcast_element_times_scalar(geom%h)

            call d_phi_d_mu_space(2)%sparse_add(term1)
            call d_phi_d_mu_space(2)%sparse_add(term2)
            call d_phi_d_mu_space(2)%sparse_add(term3)

            deallocate(term1%elements, term2%elements, term3%elements)

            ! calc d_phi_d_mu_space(3)
            call d_phi_d_mu_space(3)%init_from_sparse_vector(int%d_hH113)
            call d_phi_d_mu_space(3)%broadcast_element_times_scalar(geom%P_ls(2))

            call term1%init_from_sparse_vector(geom%d_P_ls(2))
            call term1%broadcast_element_times_scalar(int%hH113)

            call term2%init_from_sparse_vector(geom%d_h)
            call term2%broadcast_element_times_scalar(int%H123)

            call term3%init_from_sparse_vector(int%d_H123)
            call term3%broadcast_element_times_scalar(geom%h)

            call d_phi_d_mu_space(3)%sparse_add(term1)
            call d_phi_d_mu_space(3)%sparse_add(term2)
            call d_phi_d_mu_space(3)%sparse_add(term3)

            deallocate(term1%elements, term2%elements, term3%elements)
            
            call sparse_matrix1%init_from_sparse_vectors(d_phi_d_mu_space(1), d_phi_d_mu_space(2), d_phi_d_mu_space(3))
            inf_adjoint_term1 = sparse_matrix1%broadcast_matmul_element_times_3x3(this%T_mu)

            call d_T_mu_3D%init_from_sparse_matrices(this%d_T_mu_rows)
            inf_adjoint_matrix = d_T_mu_3D%broadcast_matmul_1x3_times_3row(phi_d_mu_space)
