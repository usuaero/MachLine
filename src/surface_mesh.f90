...
module surface_mesh_mod
    ...
    type, extends(mesh) :: surface_mesh
        ...
        contains
            ...
            procedure :: get_cp_locs_vertex_based_interior_adjoint => surface_mesh_get_cp_locs_vertex_based_interior_adjoint
...
contains
    ...
    subroutine surface_mesh_init_vertex_clone(this, i_jango, i_boba, mirrored_is_unique, panels_for_this_clone)
        ...
        ! if adjoint, transfer d_loc and d_n_g over
        if (this%calc_adjoint) then
            call this%vertices(i_boba)%d_loc%init_from_sparse_matrix(this%vertices(i_jango)%d_loc)
            call this%vertices(i_boba)%d_n_g%init_from_sparse_matrix(this%vertices(i_jango)%d_n_g)
        end if
    ...
    function surface_mesh_get_clone_control_point_dir(this, i_vert) result(dir)
        ...
        tp_loop: do j=1,this%vertices(i_vert)%panels_not_across_wake_edge%len()
            ...
            ! write(*,*) "vert i = ", i_vert
            ! write(*,*) "Panels not across wake = ", i_panel, iplus1_panel
            ! write(*,*) "panel verts indices",this%panels(i_panel)%get_vertex_index(1),&
            ! this%panels(i_panel)%get_vertex_index(2),&
            ! this%panels(i_panel)%get_vertex_index(3)
            ! write(*,*) "panel verts indices",this%panels(iplus1_panel)%get_vertex_index(1),&
            ! this%panels(iplus1_panel)%get_vertex_index(2),&
            ! this%panels(iplus1_panel)%get_vertex_index(3)

            ! write(*,*) ""
            ! write(*,*) "this perturb point = ", this%perturb_point
            ! write(*,*) "this calc adjoint = ", this%calc_adjoint
    ...
    subroutine surface_mesh_place_internal_vertex_control_points(this, offset, offset_type, freestream)
        ...
        if (this%calc_adjoint)then
            allocate(cp_locs(3,this%N_verts))
            d_cp_locs%num_rows= this%N_verts
            allocate(d_cp_locs%rows(this%N_verts))
            call this%get_cp_locs_vertex_based_interior_adjoint(offset, offset_type, freestream, cp_locs, d_cp_locs)
        else
    ...
    subroutine surface_mesh_init_adjoint(this)
        ...
        do i=1,this%N_original_verts
            ! init vertex sensitivities
            call this%vertices(i)%init_adjoint(this%N_original_verts)
        end do
        ...
        this%adjoint_size = this%N_original_verts*3
    ...
    subroutine surface_mesh_calc_d_vertex_geometry(this)
        ...
        do i=1,this%N_original_verts
            ...
            ! call d_n_avg%init(this%adjoint_size)  ! dont need this I think
            call sum_d_n_avg%init(this%adjoint_size)
    ...
    function surface_mesh_get_d_v_inner_at_point_wrt_vars(this, point, d_point, freestream) &
        ...
        integer :: j, k
        type(sparse_matrix) :: d_v_d_panel_wrt_vars
        ...
        do k=1,this%N_panels
            ...
            call this%panels(k)%calc_d_v_d_wrt_vars(point, d_point, freestream, .false., &
                                                ...
                                                this%asym_flow, d_v_d_panel_wrt_vars)
            ...
            call d_V_inner_wrt_vars%sparse_add(d_v_d_panel_wrt_vars)
        end do
    ...
    function surface_mesh_get_d_v_inner_at_point_wrt_mu(this, point, freestream) &
        ...
        integer :: i, j, k
        real,dimension(:,:),allocatable :: v_s_panel, v_d_panel
        type(sparse_matrix) :: d_mu_wrt_mu, d_v_d_panel_wrt_mu, d_V_inner_wrt_mu
        type(sparse_vector), dimension(3) :: d_mu_vecs
        ...
        do i=1,this%N_panels
            ...
            deallocate(d_mu_vecs(1)%elements, d_mu_vecs(2)%elements, d_mu_vecs(3)%elements, &
                d_mu_wrt_mu%columns, d_v_d_panel_wrt_mu%columns)
        end do
    ...
    function surface_mesh_get_clone_control_point_dir_adjoint(this, i_vert) result(d_dir_final)
        ...
        class(surface_mesh),intent(inout) :: this
        integer,intent(in) :: i_vert
        type(sparse_matrix) :: d_dir_final
        ...
        ! adjoint tp_normed
        ! low d hi
        call d_tp_normed%init_from_sparse_matrix(d_tp)
        call d_tp_normed%broadcast_element_times_scalar(norm2(tp))
        ...
        ! finish quotient rule
        call d_tp_normed%sparse_subtract(hi_d_low)
        call d_tp_normed%broadcast_element_times_scalar(1./(norm2(tp)*norm2(tp)))
        ...
        do j=1,this%vertices(i_vert)%panels%len()
            ...
            do k=j+1,this%vertices(i_vert)%panels%len()
                ...
                ! Get minimum angle
                y = inner(this%panels(panel1)%n_g, this%panels(panel2)%n_g)
                C_min_panel_angle = min(C_min_panel_angle, y)

                ! if the minimum angel is updated, calculate the derivative
                if (abs(C_min_panel_angle - y) < 1.0e-12) then
            ...
            if (this%mirrored .and. this%vertices(i_vert)%on_mirror_plane) then
                ...
                stop
                y = -this%panels(panel1)%n_g(this%mirror_plane)
                C_min_panel_angle = min(C_min_panel_angle, y)
        ...
        do j=1,this%vertices(i_vert)%panels_not_across_wake_edge%len()
            ...
            ! get d_n_avg 
            d_n_avg = this%panels(i_panel)%calc_d_weighted_normal(this%vertices(i_vert)%loc)
        ...
        !!!! calc derivative of normalized average normal vector !!!!!!!
        ...
        ! Determine offset ratio
        offset_ratio = 0.5*sqrt(0.5*(1. + C_min_panel_angle))

        !!! calc derivative of offset ratio !!!
        call d_offset_ratio%init_from_sparse_vector(d_C_min)
        call d_offset_ratio%broadcast_element_times_scalar(1./(8.*sqrt(0.5*(1. + C_min_panel_angle))))
        !!! end calc derivative of offset ratio !!!

        ! Determine direction
        dir = tp_normed - offset_ratio*n_avg

        !!! calc derivative of dir !!!
        call d_dir%init_from_sparse_matrix(d_tp_normed)

        d_dir_term2 = d_offset_ratio%broadcast_element_times_vector(n_avg)
        
        call d_dir_term3%init_from_sparse_matrix(d_n_avg_final)
        call d_dir_term3%broadcast_element_times_scalar(offset_ratio)

        call d_dir%sparse_subtract(d_dir_term2)
        call d_dir%sparse_subtract(d_dir_term3)

        deallocate(d_dir_term2%columns, d_dir_term3%columns)
        !!! end calc derivative of dir !!!
        ...
        !!! calc derivative of normalized dir !!!
        norm_of_dir = norm2(dir)
        
        ! calc d_norm_of_n_avg
        d_norm_of_dir = d_dir%broadcast_vector_dot_element(dir)
        call d_norm_of_dir%broadcast_element_times_scalar(1./norm_of_dir)
        
        ! adjoint Normalized dir_final 
        call d_dir_final%init_from_sparse_matrix(d_dir)
        call d_dir_final%broadcast_element_times_scalar(norm_of_dir)
        
        hi_d_low = d_norm_of_dir%broadcast_element_times_vector(dir)
        
        call d_dir_final%sparse_subtract(hi_d_low)
        call d_dir_final%broadcast_element_times_scalar(1./(norm_of_dir*norm_of_dir))

        ! deallocate stuff
        deallocate(d_dir%columns, hi_d_low%columns, d_norm_of_dir%elements)

        !!! end calc derivative of normalized dir !!!

        ! Normalize dir (isn't used again)
        dir = dir/norm2(dir)
