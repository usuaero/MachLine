program wake_test18
    ...
    allocate(C_P_inc_up(N_original_verts*3))
    allocate(C_P_inc_dn(N_original_verts*3))
    allocate(d_C_P_inc_FD(N_original_verts*3))
    ...
    write(*,*) ""
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) "               dirichlet d_CP_inc_wrt_vars TEST (WAKE PRESENT)                   "
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) ""
    write(*,*) ""
    ...
    do z =1,N_panels
        ...
        write(*,*) ""
        write(*,*) "--------------------------------------------------------------------------------------"
        write(*,'(A,I5)') "              dirichlet d_CP_inc_wrt_vars (WAKE PRESENT) test ",z
        write(*,*) "--------------------------------------------------------------------------------------"
        write(*,*) ""
        ...
        do i=1,3
            do j=1,N_original_verts
                ...
                ! get the needed info
                C_P_inc_up(j + (i-1)*N_original_verts) = test_mesh%C_P_inc(z)
                ...
                ! get the needed info
                C_P_inc_dn(j + (i-1)*N_original_verts) = test_mesh%C_P_inc(z)
    ...
    !!!!!!!!!!!!!!  RESULTS!!!!!!!!!!!!!
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) "               dirichlet d_CP_inc_wrt_vars TEST RESULTS (WAKE PRESENT) "
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) ""
