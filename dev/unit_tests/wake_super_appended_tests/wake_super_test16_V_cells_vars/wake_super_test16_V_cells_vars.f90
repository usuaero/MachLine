program wake_super_test16
    ...
    allocate(V_cells_up(3,N_original_verts*3))
    allocate(V_cells_dn(3,N_original_verts*3))
    allocate(d_V_cells_FD(3,N_original_verts*3))
    ...
    write(*,*) ""
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) "      SUPERSONIC dirichlet d_V_cells_wrt_vars TEST  (WAKE PRESENT)                  "
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) ""
    write(*,*) ""
    ...
    do z =1,N_panels
        ...
        write(*,*) ""
        write(*,*) "--------------------------------------------------------------------------------------"
        write(*,'(A,I5)') "    SUPERSONIC dirichlet d_V_cells_wrt_vars (WAKE PRESENT) test ",z
        write(*,*) "--------------------------------------------------------------------------------------"
        write(*,*) ""
        ...
        do i=1,3
            do j=1,N_original_verts
                ...
                ! get the needed info
                V_cells_up(:,j + (i-1)*N_original_verts) = test_mesh%V_cells(:,z)
                ...
                ! get the needed info
                V_cells_dn(:, j + (i-1)*N_original_verts) = test_mesh%V_cells(:,z)
        ...
        if (maxval(abs(residuals3(:,:)))>error_allowed) then
            ...
            do i = 1, N_original_verts*3
                if (any(abs(residuals3(:,i))>error_allowed)) then
                    ...
                    adjoint_mesh%d_V_cells_wrt_vars(z)%get_values(i) 
    ...
    !!!!!!!!!!!!!!  RESULTS!!!!!!!!!!!!!
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) "        SUPERSONIC dirichlet d_V_cells_wrt_vars TEST RESULTS  (WAKE PRESENT)"
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) ""
