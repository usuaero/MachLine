program wake_super_test14
    ...
    allocate(v_d_up(3,N_original_verts*3))
    allocate(v_d_dn(3,N_original_verts*3))
    allocate(d_v_d_FD(3,N_original_verts*3))
    ...
    write(*,*) ""
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) "       Supersonic dirichlet d_V_inner_wrt_vars TEST (WAKE PRESENT)                   "
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) ""
    write(*,*) ""
    ...
    do z =1,N_panels
        ...
        write(*,*) ""
        write(*,*) "--------------------------------------------------------------------------------------"
        write(*,'(A,I5)') "        Supersonic dirichlet d_V_inner_wrt_vars (WAKE PRESENT) test ",z
        write(*,*) "--------------------------------------------------------------------------------------"
        write(*,*) ""
        ...
        do i=1,3
            do j=1,N_original_verts
                ...
                ! get the needed info
                v_d_up(:,j + (i-1)*N_original_verts) = test_mesh%V_cells_inner(:,z)
                ...
                ! get the needed info
                v_d_dn(:, j + (i-1)*N_original_verts) = test_mesh%V_cells_inner(:,z)
    ...
    !!!!!!!!!!!!!!  RESULTS!!!!!!!!!!!!!
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) "       Supersonic dirichlet d_V_inner_wrt_vars TEST RESULTS (WAKE PRESENT) "
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) ""
