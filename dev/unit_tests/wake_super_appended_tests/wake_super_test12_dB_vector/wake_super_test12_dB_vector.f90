program wake_super_test12
    ...
    integer :: i,j,k,m,n,y,z, N_original_verts, N_total_verts, N_panels, vert, index, cp_ind, stat, ind
    ...
    allocate(b_up(N_original_verts*3))
    allocate(b_dn(N_original_verts*3))
    allocate(d_b_FD(N_original_verts*3))
    ...
    write(*,*) ""
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) "             SUPERSONIC d_b vector TEST (WAKE PRESENT)                   "
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) ""
    write(*,*) ""
    ...
    do z = 1,N_total_verts
        ...
        do i=1,3
            do j=1,N_original_verts
                ...
                ! get the needed info
                b_up(j + (i-1)*N_original_verts) = test_solver%b(ind)
                ...
                ! get the needed info
                b_dn(j + (i-1)*N_original_verts) = test_solver%b(ind)
    ...
    !!!!!!!!!!!!!!  RESULTS!!!!!!!!!!!!!
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) "          SUPERSONIC d_b VECTOR TEST RESULTS (WAKE PRESENT)"
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) ""
