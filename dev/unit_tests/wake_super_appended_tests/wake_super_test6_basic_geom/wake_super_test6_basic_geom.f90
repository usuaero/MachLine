program wake_super_test6
    ...
    integer :: i,j,k,m,n,p,y,z, N_original_verts, N_total_verts, N_panels, vert, index, cp_ind
    ...
    allocate(h_up(N_original_verts*3))
    allocate(h_dn(N_original_verts*3))
    allocate(d_h_FD(N_original_verts*3))
    allocate(h2_up(N_original_verts*3))
    allocate(h2_dn(N_original_verts*3))
    allocate(d_h2_FD(N_original_verts*3))
    allocate(P_g_up(3,N_original_verts*3))
    allocate(P_g_dn(3,N_original_verts*3))
    allocate(d_P_g_FD(3,N_original_verts*3))
    allocate(v_xi_up(3,N_original_verts*3))
    allocate(v_xi_dn(3,N_original_verts*3))
    allocate(d_v_xi_FD(3,N_original_verts*3))
    allocate(v_eta_up(3,N_original_verts*3))
    allocate(v_eta_dn(3,N_original_verts*3))
    allocate(d_v_eta_FD(3,N_original_verts*3))
    allocate(P_ls_up(2,N_original_verts*3))
    allocate(P_ls_dn(2,N_original_verts*3))
    allocate(d_P_ls_FD(2,N_original_verts*3))
    allocate(d_ls_up(2,N_original_verts*3))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! CALC BASIC GEOM (CONTROL POINTS) SENSITIVITIES TEST !!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do y=1,N_panels ! loop through panels
        ...
        do z= 1,N_total_verts ! loop through control points
            cp_ind  = z
            ...
            do i=1,3
                do j=1,N_original_verts
                    ...
                    ! get desired info
                    h_up(j + (i-1)*N_original_verts) = test_geom%h
                    h2_up(j + (i-1)*N_original_verts) = test_geom%h2
                    
                    P_g_up(:,j + (i-1)*N_original_verts) = test_geom%P_g(:)
                    v_xi_up(:,j + (i-1)*N_original_verts) = test_geom%v_xi(:)
                    v_eta_up(:,j + (i-1)*N_original_verts) = test_geom%v_eta(:)
                    ...
                    ! get desired info
                    h_dn(j + (i-1)*N_original_verts) = test_geom%h
                    h2_dn(j + (i-1)*N_original_verts) = test_geom%h2

                    P_g_dn(:,j + (i-1)*N_original_verts) = test_geom%P_g(:)
                    v_xi_dn(:,j + (i-1)*N_original_verts) = test_geom%v_xi(:)
                    v_eta_dn(:,j + (i-1)*N_original_verts) = test_geom%v_eta(:)
                    
                    P_ls_dn(:,j + (i-1)*N_original_verts) = test_geom%P_ls(:)
            ...
            do p=1,3
                ...
                do i=1,3
                    do j=1,N_original_verts
                        ...
                        ! get desired info
                        d_ls_up(:,j + (i-1)*N_original_verts) = test_geom%d_ls(:,p)
                        ...
                        ! get desired info
                        d_ls_dn(:,j + (i-1)*N_original_verts) = test_geom%d_ls(:,p)
