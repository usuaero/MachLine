program d_C_P_inc_test
    ...
    do k=1,test_mesh%N_panels   
        do i=1,3
            do j=1,N_verts
                ...
                ! get the needed info
                C_P_inc_up(j + (i-1)*N_verts) = test_mesh%C_P_inc(k)
                ...
                ! get the needed info
                C_P_inc_dn(j + (i-1)*N_verts) = test_mesh%C_P_inc(k)
        ...
        if (test_failed) then
            ...
            failure_log(total_tests-passed_tests) = "d_C_P_inc_test TEST FAILURE"
            write(*,*) failure_log(total_tests-passed_tests)
        else
            write(*,'(A, I1,A)') " d_C_P_inc index ",k," TEST PASSED"
    ...
    !!!!!!!!!!!!!! d_C_P_inc_test RESULTS!!!!!!!!!!!!!
    write(*,*) "-------------d_C_P_inc_test RESULTS--------------"
    write(*,*) ""
