program calc_inf_adjoint_test2
    ...
    integer :: i,j,k,m,n,p,y,z,q, N_verts, N_panels, vert, index, cp_ind
    ...
    do y= 1,N_verts
        ...
        do z=1,N_panels
            index = z
            deallocate(adjoint_mesh%edges)
            deallocate(adjoint_mesh%vertex_ordering)
            deallocate(adjoint_mesh%cp)
            deallocate(adjoint_solver%sigma_known)
            deallocate(adjoint_solver%P)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!! TEST2 CALC inf_adjoint (panel x, cp x) row 1 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            do k=1,3
                ...
                write(*,'(A, I4,A,I4,A,I4,A)') "---------------------------------- TEST2 CALC inf_adjoint &
                ...
                ! write(*,*) "--------------------------------------------------------------------------"
                ! write(*,'(A, I4,A,I4,A,I4,A)') "  CENTRAL DIFFERENCE CALC inf_adjoint2 &
                ...
                write(*,*) ""
                write(*,*) "---------------------------------------------------------------------------"
                write(*,'(A, I4,A,I4,A,I4,A)') "  ADJOINT  inf_adjoint2, (panel ",index,", panel vertex ",k, &
                ", cp,",cp_ind,")"
                write(*,*) "---------------------------------------------------------------------------"
                write(*,*) ""
                ...
                ! !write sparse matrix
                ! write(*,*) ""
                ! write(*,'(A, I4,A,I4,A,I4,A)') "  adjoint CALC inf_adjoint2 &
                ...
                if (maxval(abs(residuals))>error_allowed) then
                    ...
                    do i = 1, N_verts*3
                        if (abs(residuals(i))>error_allowed) then
                            ...
                            inf_adjoint2(k)%get_value(i), d_inf_FD(i),residuals(i)
                            maxRs(total_tests+1-passed_tests, 1)= inf_adjoint2(k)%get_value(i)
                            maxRs(total_tests+1-passed_tests, 2)= d_inf_FD(i)
                ...
                if (test_failed) then
                    ...
                    write(*,'(A, I4,A,I4,A,I4,A)') "CALC inf_adjoint2 (panel ",index,", panel vertex ",&
    ...
    write(*,*) ""
    write(*,'(I4,a9,I5,a14)') passed_tests, " out of ", total_tests, " tests PASSED"
