program d_V_cells_test
    ...
    do k=1,test_mesh%N_panels   
        do i=1,3
            do j=1,N_verts
                ...
                ! get the needed info
                V_cells_up(:,j + (i-1)*N_verts) = test_mesh%V_cells(:,k)
                ...
                ! get the needed info
                V_cells_dn(:, j + (i-1)*N_verts) = test_mesh%V_cells(:,k)
    ...
    ! write results
    write(*,*) ""
    write(*,'(A,I1)') "              Central Diff  d_V_cells_test index = ",index
    write(*,*) "  d_V_inner_x           d_V_inner_y           d_V_inner_z "
    do i = 1, N_verts*3
        write(*, '(3(f16.10, 4x))') d_V_cells_FD(:,i)
    end do 
    
    ! !write sparse matrix
    ! write(*,*) ""
    ! write(*,'(A,I1)') "                d_V_cells_test index = ",index
    ...
    do i=1,N_verts*3
        residuals3(:,i) = adjoint_mesh%d_V_cells(k)%get_values(i) - d_V_cells_FD(:,i)
    end do
    
    write(*,'(A,I1)') "          Adjoint      d_V_cells_test index = ",index
    write(*,*) "  d_V_cells_x           d_V_cells_y           d_V_cells_z                        residuals"
    do i = 1, N_verts*3
        write(*, '(3(f16.10, 4x),3x, 3(f16.10, 4x))') adjoint_mesh%d_V_cells(k)%get_values(i), residuals3(:,i)
    end do
    write(*,*) ""
    ...
    if (test_failed) then
        ...
        failure_log(total_tests-passed_tests) = "TEST FAILURE in d_V_cells"
        write(*,*) failure_log(total_tests-passed_tests)
    else
        write(*,'(A, I1,A)') " d_V_cells index = (",k,") test PASSED"
    ...
    !!!!!!!!!!!!!! d_V_cells_test RESULTS!!!!!!!!!!!!!
    write(*,*) "-------------d_V_cells_test RESULTS--------------"
    write(*,*) ""
