program d_V_cells_test
    ...
    do i=1,N_verts*3
        residuals3(:,i) = adjoint_mesh%d_V_cells_wrt_vars(k)%get_values(i) - d_V_cells_FD(:,i)
    end do
    
    write(*,'(A,I1)') "          Adjoint      d_V_cells_test index = ",index
    write(*,*) "  d_V_cells_x           d_V_cells_y           d_V_cells_z                        residuals"
    do i = 1, N_verts*3
        write(*, '(3(f16.10, 4x),3x, 3(f16.10, 4x))') adjoint_mesh%d_V_cells_wrt_vars(k)%get_values(i),&
         residuals3(:,i)
    end do
    write(*,*) ""
