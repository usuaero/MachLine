program d_cells_CF_wrt_vars_test
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! d_cells_CF_wrt_vars_test !!!!!!!!!!!!!!!!!!!!!!!!!!!!
    write(*,*) ""
    write(*,*) ""
    write(*,*) "------------------------------ d_cells_CF_wrt_vars_test ---&
    ------------------------------"
    write(*,*) ""
    write(*,*) ""
    write(*,*) ""
    ...
    !!!!!!!!! CENTRAL DIFFERENCE (panel 1, cp 1) d_cells_CF_wrt_vars_test column ", k, !!!!!!!!!
    write(*,*) ""
    write(*,*) "--------------------------------------------------------------------------------------"
    write(*,*) "                   d_cells_CF_wrt_vars_test "
    write(*,*) "--------------------------------------------------------------------------------------"
    ...
    do k=1,N_panels
        do i=1,3
            do j=1,N_verts
                ...
                ! get the needed info
                cells_CF_up(:,j + (i-1)*N_verts) = test_mesh%dC_F(:,k)
                ...
                ! get the needed info
                cells_CF_dn(:, j + (i-1)*N_verts) = test_mesh%dC_F(:,k)
        ...
        ! write results
        write(*,*) ""
        write(*,'(A,I1)') "              Central Diff  d_cells_CF_wrt_vars_test cell ",k
        write(*,*) "  d_cells_CF_wrt_x           d_cells_CF_wrt_y           d_cells_CF_wrt_z "
        do i = 1, N_verts*3
            write(*, '(3(f16.10, 4x))') d_cells_CF_FD(:,i)
        end do 
        ...
        if (test_failed) then
            ...
            write(*,'(A, I1,A)') " d_cells_CF_wrt_vars cell ",k," TEST PASSED"
    ...
    !!!!!!!!!!!!!! d_cells_CF_wrt_vars_test RESULTS!!!!!!!!!!!!!
    write(*,*) "-------------d_cells_CF_wrt_vars_test RESULTS--------------"
    write(*,*) ""
...
end program d_cells_CF_wrt_vars_test