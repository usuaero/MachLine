program d_V_inner_wrt_mu_test
    ...
    real,dimension(:,:),allocatable ::  residuals3, v_inner_wrt_mu_up, v_inner_wrt_mu_dn, d_v_inner_wrt_mu_FD
    ...
    !!!!!!!!!!!!!!!!!!!!! FIXED MESH !!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Initialize surface mesh
    call fixed_mesh%init(geom_settings)
    ...
    ! Perform flow-dependent initialization on the surface mesh
    call fixed_mesh%init_with_flow(freestream_flow, body_file, wake_file, formulation)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! d_V_inner_wrt_mu_test !!!!!!!!!!!!!!!!!!!!!!!!!!!!
    write(*,*) ""
    write(*,*) ""
    write(*,*) "------------------------------ d_V_inner_wrt_mu_test ---&
    ------------------------------"
    write(*,*) ""
    write(*,*) ""
    write(*,*) ""
    ...
    allocate(d_v_inner_wrt_mu_FD(3,N_verts))
    ...
    do k=1,test_mesh%N_panels   
        do i=1,1  !!!!!!!!!! THIS IS DIFFERENT FOR wrt mu
            do j=1,N_verts
                ...
                ! get the needed info
                v_inner_wrt_mu_up(:,j + (i-1)*N_verts) = test_mesh%V_cells_inner(:,k)
                ...
                ! get the needed info
                v_inner_wrt_mu_dn(:, j + (i-1)*N_verts) = test_mesh%V_cells_inner(:,k)
        ...
        ! central difference 
    d_mu_FD = (mu_up - mu_dn)/(2.*step)

    ! Calculate velocities
    P = fixed_mesh%panels(k)centr - 1.e-10*fixed_mesh%panels(k)%n_g
    
    v_d = (/0.0, 0.0, 0.0/) ! allocate zeros 
    do m=1,fixed_mesh%N_panels
        fixed_v_d = test_mesh%calc_velocity_influences(P, freestream_flow, .false., fixed_v_s, fixed_v_d)
        do n = 1,fixed_mesh%N_verts
            v_d_panel_m = matmul(fixed_v_d, d_mu_FD(m))
        end do
    ...
    d_v_inner_wrt_mu_FD = freestream_flow%v_inf + freestream_flow%U*(v_d)
    ...
    ! write results
    write(*,*) ""
    write(*,'(A,I1)') "              Central Diff  d_V_inner_wrt_mu index = ",index
    write(*,*) "  d_V_inner_x           d_V_inner_y           d_V_inner_z "
    do i = 1, N_verts*3
        write(*, '(3(f16.10, 4x))') d_v_inner_wrt_mu_FD(:,i)
    end do 
    ...
    do i=1,N_verts*3
        residuals3(:,i) = adjoint_mesh%d_V_cells_inner(k)%get_values(i) - d_v_inner_wrt_mu_FD(:,i)
    end do
    
    write(*,'(A,I1)') "          Adjoint      d_V_inner_wrt_mu index = ",index
    ...
    if (test_failed) then
        ...
        failure_log(total_tests-passed_tests) = "d_V_inner_wrt_mu TEST FAILURE"
        write(*,*) failure_log(total_tests-passed_tests)
    else
        write(*,'(A, I1,A)') " d_V_inner_wrt_mu index = (",k,") test PASSED"
    ...
    !!!!!!!!!!!!!! d_V_inner_wrt_mu_test RESULTS!!!!!!!!!!!!!
    write(*,*) "-------------d_V_inner_wrt_mu_test RESULTS--------------"
    write(*,*) ""
...
end program d_V_inner_wrt_mu_test