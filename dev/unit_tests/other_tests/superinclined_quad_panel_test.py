...
def plot_comparison(x, z, phi_dis, phi_anl, sing_type):
    ...
    # Plot potentials
    im1 = ax[1].contourf(z, x, phi_anl, levels=50, cmap='hot', vmin=min_phi, vmax=max_phi)
    im0 = ax[0].contourf(z, x, phi_dis, levels=im1.levels, cmap='hot', vmin=min_phi, vmax=max_phi)

    # Plot fractional difference
    frac_diff = np.log10(abs((phi_dis-phi_anl)/phi_anl))
    im2 = ax[3].contourf(z, x, frac_diff, levels=50, cmap='hot', vmin=-2.0, vmax=0.0)
...
if __name__=="__main__":
    ...
    panel.distribute_points(120, 120)
