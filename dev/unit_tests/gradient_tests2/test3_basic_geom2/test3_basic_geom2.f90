...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! CALC BASIC GEOM (CONTROL POINTS) SENSITIVITIES TEST !!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do y=1,N_panels ! loop through panels
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TEST CALC BASIC GEOM  d_h and d_h2 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            do i=1,N_verts*3
                if (abs(residuals(i)) > error_allowed .and. abs(d_h_FD(i))<10.0 .and. &
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TEST CALC   d_h2 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            ! check if test failed
            do i=1,N_verts*3
                if (abs(residuals(i)) > error_allowed .and. abs(d_h2_FD(i))<10.0 .and. &
                abs(residuals(i)) > error_allowed*10.0) then
                    test_failed = .true.
                    exit
                else 
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TEST CALC BASIC GEOM  d_v_xi, d_v_eta !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    
            ...
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TEST CALC BASIC GEOM  d_v_xi !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                ! check if test failed
                do i=1,N_verts*3
                    if (abs(residuals(i)) > error_allowed .and. abs(d_v_xi_FD(i))<10.0 .and. &
                    abs(residuals(i)) > error_allowed*10.0) then
                        test_failed = .true.
                        exit
                    else 
                ...
                ! check if test failed
                do i=1,N_verts*3
                    if (abs(residuals(i)) > error_allowed .and. abs(d_v_eta_FD(i))<10.0 .and. &
                    abs(residuals(i)) > error_allowed*10.0) then
                        test_failed = .true.
                        exit
                    else 
            ...
            do k=1,2
                ...
                ! check if test failed
                do i=1,N_verts*3
                    if (abs(residuals(i)) > error_allowed .and. abs(d_P_ls_FD(k,i))<10.0 .and. &
                        abs(residuals(i)) > error_allowed*10.0) then
                        test_failed = .true.
                        exit
                    else 
        ...
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TEST CALC BASIC GEOM  d_d_ls !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            do p=1,3
                ...
                do k=1,2
                    ...
                    ! check if test failed
                    do i=1,N_verts*3
                        if (abs(residuals(i)) > error_allowed .and. abs(d_d_ls_FD(i))<10.0 .and.&
                            abs(residuals(i)) > error_allowed*10.0) then 
                            test_failed = .true.
                            exit
                        else 
