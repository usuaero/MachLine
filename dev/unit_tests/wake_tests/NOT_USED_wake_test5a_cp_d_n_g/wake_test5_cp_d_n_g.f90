program wake_test5
    ...
    integer :: i,j,k,m,n,p,z, N_original_verts, N_total_verts, N_panels, vert, index, cp_ind
    ...
    allocate(cp_n_g_up(3,N_original_verts*3))
    allocate(cp_n_g_dn(3,N_original_verts*3))
    allocate(d_cp_n_g_FD(3,N_original_verts*3))
    ...
    write(*,*) ""
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) "          DIRICHLET  CP NORMAL SENSITIVITIES TEST  (WAKE PRESENT)                  "
    write(*,*) "------------------------------------------------------------------------"
    write(*,*) ""
    write(*,*) ""
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TEST CALC cp d_n_g  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do z = 1,N_total_verts ! for each control point
        cp_ind = z
        write(*,'(A,I5)') "CONTROL POINT NORMAL VECTOR TEST ", z
        ...
        do i=1,3
            do j=1,N_original_verts
                ...
                cp_n_g_up(:,j + (i-1)*N_original_verts) = test_mesh%cp(cp_ind)%n_g(:)
                ...
                test_mesh%vertices(j)%loc(i) = test_mesh%vertices(j)%loc(i) 
                ...
                cp_n_g_dn(:,j + (i-1)*N_original_verts) = test_mesh%cp(cp_ind)%n_g(:)
                
            end do 
        end do 
        
        ! central difference 
        d_cp_n_g_FD(:,:) = (cp_n_g_up(:,:) - cp_n_g_dn(:,:))/(step)
...
!!!!!!!!!!!!!! CP normal  SENSITIVITIES RESULTS!!!!!!!!!!!!!
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) "     DIRICHLET CP NORMAL SENSITIVITIES TEST RESULTS (WAKE PRESENT)"
    write(*,*) "------------------------------------------------------------------------------"
    write(*,*) ""
