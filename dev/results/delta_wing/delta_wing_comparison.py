...
def data_plot(comp_method, angles_of_attack):
    ...
    for k,AoA in enumerate(angles_of_attack):
        data_location = 'dev/results/delta_wing/delta_wing_{0}_deg_results.csv'.format(AoA)
        ...
        # Plot data
        plt.plot(data[:,x_loc]/max(data[:,x_loc]), data[:,Cp_loc],color='k', label = aoa_formatted, marker = shape[k], markersize = 3, linestyle="none")
...
if __name__=="__main__":
    ...
    # Dump
    input_file = "dev/results/delta_wing/input_files/diamond_wing_input.json"
