...
class Panel:
    ...
    def _calc_integrals(self, P):
        ...
        # Calculate geometry
        h, R1, R2, l1, l2, a, g2 = self._calc_geometry(P)
        ...
        for i in range(self.N):
            ...
            if R1[i] == 0.0 and R2[i] == 0.0: # Neither endpoint in DoD
                if self.b[i] > 0.0: # Supersonic edge
                    if g2[i] > 0.0: # Downstream
                        if np.sign(l1[i]) != np.sign(l2[i]): # Inside edge
            ...
            # Check for point on panel plane
                ...
                # Supersonic edge with at least one endpoint in
                if self.b[i] > 0.0:

                    # Calculate F111
                    F111[i] = -1.0/np.sqrt(self.b[i]) * np.arctan2(np.sqrt(self.b[i])*F1, F2)

                # Subsonic edge with at least one endpoint in
                else:
    ...
    def calc_induced_source_potential(self, P):
        ...
        # Calculate geometry
        h,_,_,_,_,a,_ = self._calc_geometry(P)
