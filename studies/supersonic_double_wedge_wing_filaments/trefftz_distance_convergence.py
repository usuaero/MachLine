...
def run_machline_for_trefftz(trefftz):
    # Storage locations
    case_name = "trefftz_{0}".format(trefftz)
    mesh_file = "studies/supersonic_double_wedge_wing_filaments/meshes/delta_wing_clustered_mesh_coarse.vtk"
    ...
    input_dict = {
        "flow": {
            "freestream_velocity": [np.cos(np.radians(5)),np.sin(np.radians(5)), 0.0 ]
        },
        "geometry": {
            "file": mesh_file,
            "spanwise_axis" : "+z",
...
if __name__=="__main__":
    ...
    # Run cases
    trefftz_distances = np.linspace(0,20,num_cases)
