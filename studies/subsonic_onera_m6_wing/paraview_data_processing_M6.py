...
filename = "studies/M6_ONERA_Comparison/M6_Onera_input_settings.json"
        ...
        for calc in calculations:
            ...
            for k, AoA in enumerate(AoA_folder):
                ...
                if data == "MachLine":
                    ...
                    vtk_file_location = f'studies/M6_ONERA_Comparison/vtk_Results/{data}/{calc}/' + PassedName
                elif data == "FlightStream":
                    PassedName= f"{data}_mach_" + mach_numbers[k] + "_AoA_" + AoA + ".vtk"
                    vtk_file_location = f'studies/M6_ONERA_Comparison/vtk_Results/{data}/' + PassedName
                else:
                ...
                for i, percent_semispan in enumerate(locations):
                    ...
                    if AoA in locations[percent_semispan]:
                        ...
                        # save data
                        if data == "MachLine":
                            save_location = f"studies/M6_ONERA_Comparison/csv_file_data/{calc}/M6_{data}_data_{percent_semispan}_percent_semispan_mach_{mach_numbers[k]}_AoA_{AoA}"'.csv'
                        else:
                            save_location = f"studies/M6_ONERA_Comparison/{data}_Data/M6_{data}_data_{percent_semispan}_percent_semispan_mach_{mach_numbers[k]}_AoA_{AoA}"'.csv'
