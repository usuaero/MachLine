...
def run_machline_for_grid(naca, chord_count, span_count, study_directory, index, wake_type, freestream_mach):
    ...
    #generate geometry
    naca = 2412
    area = gen_conventional_naca_wing_geom(chord_count,span_count,naca,study_directory,index)

    # Storage locations
    ## change name for new studies
    mesh_file = study_directory+"/meshes/conventional_NACA_{0}.stl".format(index) # changed name after meshes         
    results_file = study_directory+"/results/conventional_NACA"+str(index)+".vtk"
    wake_file = study_directory+"/results/conventional_NACA"+str(index)+"_wake.vtk"
    report_file = study_directory+"/reports/conventional_NACA"+str(index)+".json"
...
if __name__=="__main__":
    ...
    output_file += "/grid_convergence_conventional_NACA.txt"
