...
def plot_comparison(M, alphas, grid, se_pressures, reports):
    ...
    for i, case in enumerate(cases):
        ...
        C_p_ise_2_ML_std = np.zeros(len(alphas))
        C_p_ise_3_ML_std = np.zeros(len(alphas))
        C_p_ise_4_ML_std = np.zeros(len(alphas))
        C_p_ise_5_ML_std = np.zeros(len(alphas))
        C_p_2nd_2_ML_std = np.zeros(len(alphas))
        C_p_2nd_3_ML_std = np.zeros(len(alphas))
        C_p_2nd_4_ML_std = np.zeros(len(alphas))
        C_p_2nd_5_ML_std = np.zeros(len(alphas))
        C_p_sln_2_ML_std = np.zeros(len(alphas))
        C_p_sln_3_ML_std = np.zeros(len(alphas))
        C_p_sln_4_ML_std = np.zeros(len(alphas))
        C_p_sln_5_ML_std = np.zeros(len(alphas))
        C_p_lin_2_ML_std = np.zeros(len(alphas))
        C_p_lin_3_ML_std = np.zeros(len(alphas))
        C_p_lin_4_ML_std = np.zeros(len(alphas))
        C_p_lin_5_ML_std = np.zeros(len(alphas))
        ...
        for j, alpha in enumerate(alphas):
            ...
            C_p_ise_2_ML_std[j] = np.std(C_p_ise[ind2])
            C_p_ise_3_ML_std[j] = np.std(C_p_ise[ind3])
            C_p_ise_4_ML_std[j] = np.std(C_p_ise[ind4])
            C_p_ise_5_ML_std[j] = np.std(C_p_ise[ind5])
            C_p_2nd_2_ML_std[j] = np.std(C_p_2nd[ind2])
            C_p_2nd_3_ML_std[j] = np.std(C_p_2nd[ind3])
            C_p_2nd_4_ML_std[j] = np.std(C_p_2nd[ind4])
            C_p_2nd_5_ML_std[j] = np.std(C_p_2nd[ind5])
            C_p_sln_2_ML_std[j] = np.std(C_p_sln[ind2])
            C_p_sln_3_ML_std[j] = np.std(C_p_sln[ind3])
            C_p_sln_4_ML_std[j] = np.std(C_p_sln[ind4])
            C_p_sln_5_ML_std[j] = np.std(C_p_sln[ind5])
            C_p_lin_2_ML_std[j] = np.std(C_p_lin[ind2])
            C_p_lin_3_ML_std[j] = np.std(C_p_lin[ind3])
            C_p_lin_4_ML_std[j] = np.std(C_p_lin[ind4])
            C_p_lin_5_ML_std[j] = np.std(C_p_lin[ind5])

        # Plot data from MachLine
        #plt.errorbar(alphas, C_p_ise_2_ML, yerr=C_p_ise_2_ML_std, fmt='kv', markersize=3, label="Isentropic")
        #plt.errorbar(alphas, C_p_ise_3_ML, yerr=C_p_ise_3_ML_std, fmt='kv', markersize=3)
        #plt.errorbar(alphas, C_p_ise_4_ML, yerr=C_p_ise_4_ML_std, fmt='kv', markersize=3)
        #plt.errorbar(alphas, C_p_ise_5_ML, yerr=C_p_ise_5_ML_std, fmt='kv', markersize=3)
        #plt.errorbar(alphas, C_p_2nd_2_ML, yerr=C_p_2nd_2_ML_std, fmt='ks', markersize=3, label="Second-Order")
        #plt.errorbar(alphas, C_p_2nd_3_ML, yerr=C_p_2nd_3_ML_std, fmt='ks', markersize=3)
        #plt.errorbar(alphas, C_p_2nd_4_ML, yerr=C_p_2nd_4_ML_std, fmt='ks', markersize=3)
        #plt.errorbar(alphas, C_p_2nd_5_ML, yerr=C_p_2nd_5_ML_std, fmt='ks', markersize=3)
        #plt.errorbar(alphas, C_p_sln_2_ML, yerr=C_p_sln_2_ML_std, fmt='ko', markersize=3, label="Slender-Body")
        #plt.errorbar(alphas, C_p_sln_3_ML, yerr=C_p_sln_3_ML_std, fmt='ko', markersize=3)
        #plt.errorbar(alphas, C_p_sln_4_ML, yerr=C_p_sln_4_ML_std, fmt='ko', markersize=3)
        #plt.errorbar(alphas, C_p_sln_5_ML, yerr=C_p_sln_5_ML_std, fmt='ko', markersize=3)
        #plt.errorbar(alphas, C_p_lin_2_ML, yerr=C_p_lin_2_ML_std, fmt='k^', markersize=3, label="Linear")
        #plt.errorbar(alphas, C_p_lin_3_ML, yerr=C_p_lin_3_ML_std, fmt='k^', markersize=3)
        #plt.errorbar(alphas, C_p_lin_4_ML, yerr=C_p_lin_4_ML_std, fmt='k^', markersize=3)
        #plt.errorbar(alphas, C_p_lin_5_ML, yerr=C_p_lin_5_ML_std, fmt='k^', markersize=3)
